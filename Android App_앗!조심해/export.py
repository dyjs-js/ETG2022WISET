...
def export_torchscript(model, im, file, optimize, prefix=colorstr('TorchScript:')):
    ...
    fl = file.with_suffix('.torchscript.ptl') #add by sun103

    ts = torch.jit.trace(model, im, strict=False)
    d = {"shape": im.shape, "stride": int(max(model.stride)), "names": model.names}
    extra_files = {'config.txt': json.dumps(d)}  # torch._C.ExtraFilesMap()
    (optimize_for_mobile(ts) if optimize else ts).save(str(f), _extra_files=extra_files) #add by sun103
    (optimize_for_mobile(ts) if optimize else ts)._save_for_lite_interpreter(str(fl), _extra_files=extra_files) #add by sun103
    if optimize:  # https://pytorch.org/tutorials/recipes/mobile_interpreter.html
        optimize_for_mobile(ts)._save_for_lite_interpreter(str(f), _extra_files=extra_files)
    else:
        ts.save(str(f), _extra_files=extra_files)
    return fl, None # modify by sun103 f --> fl
